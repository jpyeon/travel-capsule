  ...
  return (
    <main className="max-w-4xl mx-auto p-6 space-y-6">
      ...
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-bold text-gray-900 dark:text-night-50">My Trips</h1>
      ...
      {loading && (
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
          {[...Array(2)].map((_, i) => (
            <div key={i} className="h-40 animate-pulse rounded-xl bg-sand-100 dark:bg-night-200" />
          ))}
        </div>
      )}
      ...
      {!loading && !error && trips.length === 0 && (
        <div className="flex flex-col items-center gap-4 rounded-xl border border-sand-200 dark:border-night-100 bg-white dark:bg-night-200 py-20 text-center shadow-card">
          <p className="text-sm font-medium text-gray-700 dark:text-sand-300">No trips planned yet.</p>
